...
export function answerChat(message: string, earthquakes: Earthquake[]) {
  const lowered = normalizeText(message)
  const now = Date.now()
  const windowMinutes = getWindowMinutes(message)
  const windowHours = windowMinutes === null ? null : Math.max(1, Math.round(windowMinutes / 60))
  ...
  if (lowered.includes('hangi il') || lowered.includes('hangi bolge') || lowered.includes('hangi bölge') || lowered.includes('en yogun')) {
